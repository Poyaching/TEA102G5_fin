...
<%
	...
	Verify_listService Src =new Verify_listService();
%>
			...
			<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/verify_list/verify_listServlet"
				...
				<table class="table table-striped" style="border-style:dotted; border-width:0.5px">
					...
					<tr>
						<td>課程名稱:</td>
						<% 
						String getClass_name= class_infoVO.getClass_name();
						
						if (getClass_name == null){
							getClass_name = "無資料";
						}else{
							class_infoVO.getClass_name();
						}
						
						%>
						<td><input readonly class="form-control" type="TEXT" name="class_name" size="45"
							value="<%=getClass_name%>" /></td>
					</tr>
					<tr>
						<td>會員編號:</td>
					...
					<tr>
						...
						if (getClass_description == null){
							...
							class_infoVO.getClass_description();
						}
						
						%>
						<td><textarea readonly class="form-control" rows="5" cols="50" name="class_description"><%=getClass_description%></textarea>
						</td>
					</tr>
					<tr>
						<td>募資售價:</td>
					...
					<tr>
						<td>課程長度:</td>
						<% 
						String getClass_length= class_infoVO.getClass_length();
						
						if (getClass_length == null){
							getClass_length = "無資料";
						}else{
							class_infoVO.getClass_length();
						}
						
						%>
						<td><input readonly class="form-control" type="TEXT" name="class_length" size="45"
							value="<%=getClass_length%>" /></td>
					</tr>
					<tr>
						<td>課程封面圖片:</td>
