...
<%
	Login_historyVO login_historyVO = (Login_historyVO) request.getAttribute("login_historyVO"); //EmpServlet.java (Concroller) 存入req的empVO物件 (包括幫忙取出的empVO, 也包括輸入資料錯誤時的empVO物件)
%>
<html>
<head>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1" />
<title>登入歷史紀錄修改</title>
...
<body bgcolor='white'>
	...
	<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/login_history/login_historyServlet" name="form1">
		<table>
			<tr>
				<td>登入編號:<font color=red><b>*</b></font></td>
				<td><%=login_historyVO.getLogin_id()%></td>
			</tr>
			<tr>
				<td>登入時間:</td>
				<td><input name="login_time" id="f_date1" type="text"></td>
			</tr>

			<tr>
				<td>登入裝置:</td>
				<td><input type="TEXT" name="login_arrange" size="45" value="<%=login_historyVO.getLogin_arrange()%>" /></td>
			</tr>
			<tr>
				<td>登入IP:</td>
				<td><input type="TEXT" name="login_ip" size="45" value="<%=login_historyVO.getLogin_ip()%>" /></td>
			</tr>
		...
		<br> <input type="hidden" name="action" value="update"> <input
			type="hidden" name="login_id" value="<%=login_historyVO.getLogin_id()%>"> <input
