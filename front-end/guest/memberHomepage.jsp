...
<body>
	...
	Member_infoVO  guestmember_infoVO = srv.getOneMember_info(member_id);
	...
	<div class="container">
		...
		<div id="content">
			<div class="row">
				...
				<div class="col-3">
					<div class="card">
						<img
							src="<%=request.getContextPath()%>/guest/guestMember_info_frontServlet?action=member_pic&member_id=${guestmember_infoVO.member_id}"
							class="card-img-top" alt="...">
						<div class="card-body">
							<p style="text-align: center;"><%=guestmember_infoVO.getMember_name()%></p>
						</div>
					</div>
					...
					<div class="card border-dark mb-3" style="max-width: 18rem;">
						...
						<div class="card-body text-dark">
							<h9 class="card-title"><%=guestmember_infoVO.getMember_about()%></h9>
						</div>
					</div>
					<div class="card border-dark mb-3" style="max-width: 18rem;">
						<div class="card-header">我的興趣</div>
						<div class="card-body text-dark">
							<h9 class="card-title">語言學習</h9>
						</div>
					</div>
					<div class="card border-dark mb-3" style="max-width: 18rem;">
						<div class="card-header">我的專長</div>
						<div class="card-body text-dark">
							<h9 class="card-title"><%=guestmember_infoVO.getMember_good_for()%></h9>
						</div>
					</div>
							...
							<c:forEach var="class_infoVO" items="${list}">
								<div class="card col-6">
									<img class="card-img-top"
										...
										alt="Card image cap">
									<div class="card-body">
										<h5 class="card-title">${guestmember_infoVO.class_name}</h5>
										<p class="card-text">${guestmember_infoVO.class_description}</p>
									</div>
							...
							<c:forEach var="order_listVO" items="${list2}">
								...
								<div class="card col-6">
									<img class="card-img-top"
										...
										alt="Card image cap">
									<div class="card-body">
										<h5 class="card-title">${classSrv.getOneClass_info(order_listVO.class_id).class_name}</h5>
										<p class="card-text">${classSrv.getOneClass_info(order_listVO.class_id).class_description}</p>
									</div>
		...
		<!--  聊天室-->
		<img id="talk" src="<%=request.getContextPath()%>/img/icon/chat.png">
		...
		<script>
			...
			if (<%=guestmember_infoVO.getTeachclass_on()%>==1) {
				$("#tclass").attr("checked", true);
			}
			if (<%=guestmember_infoVO.getLearnclass_on()%>==1) {
				$("#lclass").attr("checked", true);
			}
			if (<%=guestmember_infoVO.getMember_homework()%>==1) {
			...
			$("#tclass, #lclass, #stuhw").change(function() {
			    ...
			    if ($("#stuhw").prop("checked")) {
				    ...
				    var data= {
				        ...
				        "member_id": "<%=guestmember_infoVO.getMember_id()%>",
