...
@WebServlet("/member_info/Member_infoServlet")
...
//				res.sendError(HttpServletResponse.SC_NOT_FOUND);    //輸出錯誤訊息
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/member_info/select_page.jsp");
				...
				String url = "/back-end/member_info/listOneMember_info.jsp";
		...
		if ("getOne_For_Update".equals(action)) {
			...
			try {
				...
				String url = "/back-end/member_info/update_member_info_input.jsp";
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/back-end/member_info/listAllMember_info.jsp");
				failureView.forward(req, res);
			}
		}
...
//			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/member_info/update_member_info_input.jsp");
					failureView.forward(req, res);
					return; // 程式中斷
				}
				...
				member_infoVO = member_infoSvc.update(member_id, member_name, member_email, member_password,
						...
						Integer.valueOf(learnclass_on), Integer.valueOf(member_homework), member_about,
						member_good_for, register_time, member_update, traccount, bank_code);
			...
			try {
				...
				if (fis.available() != 0) {
					...
					InputStream in = getServletContext()
							.getResourceAsStream("/back-end/member_info/images/bighead.jpg");
...
//					out.write(buffer);
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("member_infoVO", member_infoVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/member_info/addMember_info.jsp");
					failureView.forward(req, res);
					return; // 程式中斷
				}
				...
				/*************************** 3.修改完成,準備轉交(Send the Success view) *************/
				String url = "/back-end/member_info/listAllMember_info.jsp";
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/back-end/member_info/addMember_info.jsp");
				failureView.forward(req, res);
			}
		}
		
		//===========================================新增user會員帳號 addMember_user.jsp-非必要驗證欄位關掉========================
...
//						}
		...
		if ("delete".equals(action)) { // 來自listAllEmp.jsp
			...
			if ("updatestatus".equals(action)) { 
			    // ===========================================個人頁面ajax===============================
				System.out.println("進入updatestatus");
				List<String> errorMsgs = new LinkedList<String>();
				req.setAttribute("errorMsgs", errorMsgs);
			    /*************************** 1.接收請求參數 - 輸入格式的錯誤處理 **********************/
			    ...
			    /*************************** 2.開始修改資料 *****************************************/
			    Member_infoService member_infoSvc = new Member_infoService();
			    member_infoVO = member_infoSvc.updatestatus(member_id, Integer.valueOf(teachclass_on),
			      Integer.valueOf(learnclass_on), Integer.valueOf(member_homework));
