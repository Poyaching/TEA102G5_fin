	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("class_Introduction".equals(action)) {
			...
			if (class_infoVO != null) { // 判斷檔案是否存在
				...
				for(Class_infoVO VO : (List<Class_infoVO>)session.getAttribute("member_teach")) {
					//是不是自己開的課
					if(VO.getClass_id().equals(class_id)) {
						req.setAttribute("class_infoVO", class_infoVO);
						System.out.println(class_infoVO.getClass_id());
						RequestDispatcher failureView = req
								.getRequestDispatcher("/front-end/class_info/classlearn.jsp"); // classOnly.jsp
																								// classlearn.jsp
...
//					int date =.compareTo( new java.util.Date().getTime());
					...
					if ((class_infoVO.getStartfund_date().getTime() + (long)30 * 24 * 60 * 60 * 1000) > new java.util.Date()
						...
						System.out.println(new Date(class_infoVO.getStartfund_date().getTime() + 30 * 24 * 60 * 60 * 1000));
