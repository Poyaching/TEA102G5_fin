...
@WebServlet("/PutMessage/PutMessage")
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if ("insert".equals(action)) {
				...
				// 將資料包裝到VO
				Main_messageVO MainMsgVO = new Main_messageVO();

				MainMsgVO.setClass_id(jsonObject.getString("class_id"));
				MainMsgVO.setMainmsg_text(jsonObject.getString("mainmsg_text"));
				MainMsgVO.setMainmsg_time(new java.sql.Timestamp(new Date().getTime()));
				MainMsgVO.setMember_id(jsonObject.getString("member_id"));
				MainMsgVO.setMsg_source(jsonObject.getString("msg_source"));
				MainMsgVO.setMsg_status(1);


				// 進入資料庫
				Main_messageService Srv = new Main_messageService();
				Srv.addMain_message(MainMsgVO);
				for(Main_messageVO item :Srv.getAll()) {
					if(item.getClass_id().equals(jsonObject.getString("class_id")) && item.getMember_id().equals(jsonObject.getString("member_id"))
					&& item.getMainmsg_time().equals(MainMsgVO.getMainmsg_time())) {
						MainMsgVO.setMainmsg_id(item.getMainmsg_id());
					}
				}
				// 回傳到前端AJAX
				JSONObject AllJsonObject = new JSONObject(MainMsgVO);
